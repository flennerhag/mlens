...
class LayerContainer(BaseEstimator):
    ...
    def fit(self, X=None, y=None, return_preds=-1, **process_kwargs):
        ...
        **process_kwargs : optional
        ...
        """
        return self._fit('fit', X, y, return_preds, **process_kwargs)

    def fit_proba(self, X=None, y=None, return_preds=-1, **process_kwargs):
        ...
        **process_kwargs : optional
        ...
        """
        return self._fit('fit_proba', X, y, return_preds, **process_kwargs)
    ...
    def predict(self, X=None, y=None, **process_kwargs):
        ...
        """
        return self._predict('predict', X, y, **process_kwargs)

    def predict_proba(self, X=None, y=None, **process_kwargs):
        r"""Predict layers using the estimators ``predict_proba`` method.
        ...
        Returns
        -------
        X_pred : array-like of shape = [n_samples, n_labels * n_fitted_ests]
            predictions from final layer.
        """
        return self._predict('predict_proba', X, y, **process_kwargs)

    def _fit(self, job, X=None, y=None, return_preds=-1, **process_kwargs):
        """Routine for fitting all layers through specified job type.

        See respective public method (fit, fit_proba) for detailed
        documentation.
        """
        ...
        # Initialize cache
        if not _init:
            processor = ParallelProcessing(self, job=job)
    ...
    def _predict(self, job, X, y, **kwargs):
        """Routine for generating predictions through all layers
        with specified job type.

        See respective public method (predict, predict_proba) for detailed
        documentation.
        """
        ...
        # Initialize cache
        if not _init:
            processor = ParallelProcessing(self, job)
            processor.initialize(X, y, **kwargs)
        else:
            processor = self._processor
