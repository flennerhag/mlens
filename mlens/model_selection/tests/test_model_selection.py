...
def test_no_prep():
    ...
    evl.fit(X, y,
            ...
            n_iter=10)
    assert evl.summary['test_score_mean']['ols'] == np.abs(-28.376957869124858)
...
def test_w_prep():
    ...
    assert evl.summary['test_score_mean'][('no', 'ols')] == 28.376957869124858
    assert evl.summary['test_score_mean'][('pr', 'ols')] == 39.007745452637408
...
def test_w_prep_set_params():
    ...
    assert evl.summary['test_score_mean'][('no', 'ols')] == 84.856535669630389
    assert evl.summary['test_score_mean'][('pr', 'ols')] == 142.9297326963281
