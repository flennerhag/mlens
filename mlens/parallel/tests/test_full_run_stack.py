...
from mlens.utils.dummy import destroy_temp_dir, _layer_est, _store_X_y, \
    data, get_layers, get_path, ground_truth
...
LAYER, LAYER_CONTAINER, LCM = get_layers('stack', False, n_splits=FOLDS)
...
def test_layer_fit():
    """[Layer] Stack: 'fit' method runs correctly."""
...
def test_layer_mmaps():
    """[Layer] Stack: no ests point to mmaps."""
...
def test_lc_fit():
    """[LayerContainer] Stack: 'fit' method runs correctly."""
...
def test_lc_mmaps():
    """[LayerContainer] Stack: no ests point to mmaps from."""
    lc = clone(LAYER_CONTAINER)
    out = lc.fit(X, y, return_preds=True)
...
def test_layer_predict():
    """[Layer] Stack: 'predict' method runs correctly."""
...
def test_lc_mmap_from_file():
    """[LayerContainer] Stack: no ests point to mmaps when fit from file."""
    path = get_path()
    X_path, y_path = _store_X_y(path, X, y)
...
def test_lc_predict_file():
    """[LayerContainer] Stack: test predict from file path."""
    path = get_path()
    X_path, y_path = _store_X_y(path, X, y)
