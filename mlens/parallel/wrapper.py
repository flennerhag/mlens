...
class EstimatorMixin(object):
    ...
    Mixin class to build an estimator from a :mod:`mlens.parallel` base
    class. The base class should be set as the ``_backend`` attribute
    of the estimator.
    """
    ...
    def predict_proba(self, X):
        ...
        Predict class probabilities using full-fold estimators
        (fitted on all data). Equivalent to ``predict`` with ``proba=True``.
        ...
        """
        return run(
            get_backend(self), 'predict', X, proba=True, return_preds=True)
...
def set_predict(kwargs):
    ...
    out = dict()
    proba = kwargs.pop('proba', None)
    if proba is not None:
        out['_proba'] = proba
        out['_attr'] = 'predict_proba' if proba else 'predict'
    return out


def set_no_output(kwargs, job):
